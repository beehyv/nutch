package com.beehyv.nutch.parse.rules;

/**
 * Created by kapil on 26/10/16.
 */
public class RuleConstants {

    public static final String DOMAIN_ID = "domainId";
    public static final String TENANT_ID = "tenantId";
    public static final String URL_REGEX = "urlRegex";
    public static final String PROPERTY = "property";
    public static final String VALUE = "value";
    public static final String VALUE_TYPE = "valueType";

    public static final String PRODUCT_ID = "productId";
    public static final String PAGE_TYPE_ENUM = "pageType";
    public static final String WHITELIST_CSS_SELS = "whitelistCssSels";
    public static final String BLACKLIST_CSS_SELS = "blacklistCssSels";
    public static final String RULESELS_SEP = ",";
    public static final String SOURCE_URL = "sourceUrl";

    public static final String FLD_TENANT = "tenant";
    public static final String FLD_TYPE = "type";

    public static final String FLD_PRODUCT_TYPE_ID = "productTypeId";

    public static final String PAGE_TYPE_TENANT_PRODUCT = "tenant_product_page";

}
